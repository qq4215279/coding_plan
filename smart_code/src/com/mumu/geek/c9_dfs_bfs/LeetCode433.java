...
public class LeetCode433 {
    ...
    // 宽搜
    private static int bfs2(Set<String> st, Set<String> ed, Set<String> menSt, Set<String> menEd, Set<String> dict, int len) {
        // 起始集合为空，那么就无法到达目标
        if(st.size() == 0) return -1;
        // 优先从数量少的一端开始搜索，减少搜索量
        if(st.size() > ed.size()) return bfs2(ed, st, menEd, menSt, dict, len);
