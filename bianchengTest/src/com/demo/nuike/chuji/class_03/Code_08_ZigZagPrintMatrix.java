...
public class Code_08_ZigZagPrintMatrix {	//“之”字形打印矩阵 =表示赋值，==表示比较
	...
	public static void printLevel(int[][] m, int aR, int aC, int bR, int bC,	//打印过程
			boolean f) {
		if (f) {
			while (aR != bR + 1) {
				System.out.print(m[aR++][aC--] + " ");	//从右上交往左下角打印(行++，列--)
			}
		} else {
			while (bR != aR - 1) {
				System.out.print(m[bR--][bC++] + " ");	//从左下角往右上角打印(行--，列++)
			}
		}
	}
