...
public class Code_08_NetherlandsFlag { // 荷兰国旗问题--快排问题  partition-分割
	...
	public static int[] partition(int[] arr, int l, int r, int num) {//思路：与当前数比较，小于放左边，等于放中间，大于放右边
		int less = l - 1;	//less表示小于区域，l表示左边界
		int more = r + 1;	//more表示大于区域，r表示右边界
		int cur = l;
		while (cur < more) {	//cur有点相当于指针，表示当前数的索引
			if (arr[cur] < num) {
				swap(arr, ++less, cur++);//++less表示小于区域的后一个数；相当于小于区域向右扩一个位置，cur指针+1
			} else if (arr[cur] > num) {
				swap(arr, --more, cur);//--more表示大于区域的前一个数
