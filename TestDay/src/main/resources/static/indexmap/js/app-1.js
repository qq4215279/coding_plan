...
for(let n=0;n<ullist.length;n++) {
  ullist[n].onclick = function () {
    if(parseInt(ullist[n].getAttribute('index'))>0 && parseInt(ullist[n].getAttribute('index'))<6) {
      if (Math.ceil(total/20)>= parseInt(ullist[n].innerText) ) {
        for(var m=2;m<7;m++) {
...
localStorage.setItem('cityname',name.split('&')[0].split('=')[1])
localStorage.setItem('cityId',deptId)
var $date = "#date";
// 左上角导航
...
cityLaw.innerText = name.split('&')[0].split('=')[1]
cityLaw.title = name.split('&')[0].split('=')[1]
...
function setDeptCountGroup (d) {
  ...
  d.forEach(function (ele, index) {
    htmlStr += '<div class="org-rank-item">' +
      ...
      (index + 1) +
      '</div>' +
      '<div class="org-name "><div class="line2-over" style="font-size:12px;cursor: pointer;" index="'+ele.deptId+'" onclick="asd(event)">' +
      ele.name +
      '</div></div>' +
      '<div class="org-rank-bar ana-trs" style="width:' + ele.value / val * 0.4 * 100 + '%">' +
      '</div>' +
...
function asd (e) {
  let aa = e.target.getAttribute('index')
  location.href='../indexmap/index-2.html?name='+e.target.innerHTML + '&id=' + aa
}
/**
 * 民族排序
 * @param {*} d 
