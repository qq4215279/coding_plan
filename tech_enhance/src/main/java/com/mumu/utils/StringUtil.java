...
public class StringUtil {
    ...
    public static String getString(String str, int pos, int value, String comma) {
        int[] arr = StringUtil.getArray(str, comma, 0);
        if (arr.length < pos) {
            int[] tmp = new int[pos];
        ...
        arr[pos - 1] = value;
        return StringUtil.getString(arr, comma);
    }
    ...
    public static int getIntervalDayNum(Date startDate, Date endDate) {
        ...
        return (int)((now.getTimeInMillis() - last.getTimeInMillis()) / DAY_SEC);
    }
