...
public class ArraysTest {
    ...
    public void apiTest() {
        ...
        // 2. 二分查找法找指定元素的索引值（下标） binarySearch(T... a, key)  注：数组一定是排好序的，否则会出错！！！找到元素，只会返回最后一个位置
        ...
        // 3.1 复制指定长度的数组 copeOf(T... original, newLength)
        int[] arrCopy01 = Arrays.copyOf(arr, 3); // [10,20,30]
        // 3.2 数组复制 copyOfRange(T... original, int from, int to)
        ...
        // 6.1 排序 sort(T... a)  注：数字从小到大排序，字母先大写后小写
        int[] intArray = new int[] {4, 1, 3, -23};
        Arrays.sort(intArray); // 输出： [-23, 1, 3, 4]
        // 6.2 忽略大小写排序 Case-insensitive sort
